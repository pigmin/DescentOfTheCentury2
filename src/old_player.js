...
class Player {
    ...
    test() {
        window.addEventListener('dblclick', () => {
            ...
            var start = GlobalManager.gameCamera.target;
    ...
    update(delta) {
        ...
        if (this.bOnGround) {
            //Inputs
            this.moveDir.scaleInPlace(this.speed);
        }
        else {
            //Inputs
            this.moveDir.scaleInPlace(this.speed * 0.75);
        }

        //On regarde le slope etc.
        this.calculateSlope();
        this.alignPlayerMove();
    ...
    alignPlayerMove() {
        ...
        if (this.onSlope) {
            ...
            /*
                        var axis1 = this.slopeHit.hitNormal;
                        let axis2 = Vector3.Cross(Vector3.UpReadOnly, this.slopeHit.hitNormal);
                        let axis3 = Vector3.Cross(axis2, this.slopeHit.hitNormal);
            
                        var start = new Vector3(Math.PI / 2, Math.PI / 2, 0);	//camera.position			
                        
                        
                        var tmpVec = Vector3.RotationFromAxis(axis3.negate(), axis1, axis2);
                        var quat = Quaternion.RotationQuaternionFromAxis(axis3.negate(), axis1, axis2);
                        this.snowboard.rotation = tmpVec;
            
                        //this.gameObject.setDirection(this.direction.applyRotationQuaternion(quat));
            ...
            //            direction = direction.Rotated(Vector3.Up.Cross(normal).Normalized(), Vector3.Up.AngleTo(normal)); // Also rotate the direction
                        ...
                        /*
                                    ...
                                    Matrix.RotationAlignToRef(Vector3.UpReadOnly, this.slopeHit.hitNormal, rotationMatrix);
                                    let rotationQuaternion = Quaternion.FromRotationMatrix(rotationMatrix);
                        
                                    let a2 = rotationQuaternion.toEulerAngles();            
                                    //console.log(a2);
                                    console.log(a2.x, this.gameObject.rotation.y, 0);
                                    
                                    this.gameObject.rotation.set(a2.x, this.gameObject.rotation.y, 0);
                        
                        // let angle2 = Vector3.GetAngleBetweenVectors(Vector3.UpReadOnly, this.slopeHit.hitNormal, this.getForwardVector(this.gameObject));
            
                        //const rotationQuaternion = Quaternion.RotationAxis(Axis.X, angle);
                        //let a2 = rotationQuaternion.toEulerAngles();            
                        //newRot.applyRotationQuaternionInPlace(rotationQuaternion);
            
                        //let ret = planarMoveDir.clone();
                        //let axis = this.getRightVector(this.gameObject);         
                        //this.gameObject.rotate(axis, this.currentSlope);
            
            
                        //On suit la pente
                        // let upright = Vector3.Cross(this.getRightVector(this.transform), this.slopeHit.hitNormal)
                        // this.gameObject.rotationQuaternion = Quaternion.FromLookDirectionLH(Vector3.Cross(this.getRightVector(this.transform), upright), Vector3.UpReadOnly);
            
                        //console.log(newRot);
                        //FromLookDirectionLHsdddddddddddddddd
                        //Double points un devant un derriere le snowboard : ideal
                        ...
                        //console.log(this.slopeHit.hitNormal, this.slopeHit.hitNormalWorld);        
    ...
    /*
        getSlopeMoveDirection(moveDir) {
            let plane = Plane.FromPositionAndNormal(Vector3.Zero(), this.slopeHit.hitNormal);
            return Vector3.projectOnPlane()
        }*/

    //Vector3 

    // methodes a etudier pour les slopes etc.  : 
    //GetAngleBetweenVectors  /  GetAngleBetweenVectorsOnPlane
    // Reflect 
    //projectOnPlane
    //const rotationQuaternion = Quaternion.RotationAxis(Axis.Z, Math.PI / 8);
    ...
    applyCameraDirectionToMoveDirection() {
        ...
        if (this.moveDir.length() != 0) {
            let forwardCamera = this.getForwardVector(GlobalManager.gameCamera);
            let rightCamera = this.getRightVector(GlobalManager.gameCamera);
