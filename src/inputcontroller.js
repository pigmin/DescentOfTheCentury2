    ...
    update(delta) {
        ...
        if (!this.bGamePadConnected) {
            ...
            if (this.inputMap["KeyA"])
                this.#axisP1.x = Scalar.MoveTowards(this.#axisP1.x, -1, 10 * delta);
            else if (this.inputMap["KeyD"])
                this.#axisP1.x = Scalar.MoveTowards(this.#axisP1.x, 1, 10 * delta);
            else
                this.#axisP1.x = Scalar.MoveTowards(this.#axisP1.x, 0, 10 * delta);


            if (this.inputMap["KeyS"])
                this.#axisP1.y = Scalar.MoveTowards(this.#axisP1.y, -1, 10 * delta);
            else if (this.inputMap["KeyW"])
                this.#axisP1.y = Scalar.MoveTowards(this.#axisP1.y, 1, 10 * delta);
            else
                this.#axisP1.y = Scalar.MoveTowards(this.#axisP1.y, 0, 10 * delta);
